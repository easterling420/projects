...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(!empty($_POST["otp"])){
        if($otp == $_SESSION["otpVal"]){
            ...
            if(!$conn){
                $successErr =  "error connecting...";
            }
            else{
                $query = "update d_users set password='$newPass' where username='$user' and password='$oldPass'";
                if($conn -> query($query)){
                    $success = "Password Updated Successfully";
                    
                }
                else{
                    $successErr = "Unknown Error Occurred";
                }
            }
            $conn->close();
        }
        else{
            $otpErr = "OTP didn't Matched";
        }
    }
    session_destroy();
    unset($_SESSION);
...
<html>
<head>
  <title> Ambey ITI - OTP Verification </title>
...
<body>
  <div class="body-container">
    ...
    <form id="contact" action="" method="POST">
        <h3>OTP Sent at Your Email</h3>
        <fieldset>
            <input placeholder="Enter OTP Sent" type="text" name="otp" tabindex="1">
        </fieldset>
        <p class="error"><?= $otpErr ?></p>
        <button name="submit" type="submit" id="pass-submit" data-submit="...Sending">Submit</button>
        <p class="success"><?= $success?></p>
        <p class="error"><?= $successErr ?></p>
    </form>
</div>
</div>
