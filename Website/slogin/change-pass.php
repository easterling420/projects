...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(!empty($_POST["user"]) && !empty($_POST["oldPass"]) && !empty($_POST["newPass"]) && !empty($_POST["email"])){
        ...
        if(!$conn){
            ...
            $query = "select username, password from d_users where username='$user' and password='$oldPass'";
            ...
            if($result->num_rows == 0){
               $otpErr = "User Doesn't Exists !!!";
                ...
                $query = "select * from d_users where username='$user' and password='$oldPass' and email is not NULL";
                $result = $conn -> query($query);
                
                if($result->num_rows == 0){
                    $query = "update d_users set email='$email' where username='$user' and password='$oldPass'";
                    $conn -> query($query);
                }
                else{
                    $query = "select email from d_users where username='$user' and password='$oldPass'";
                    $result = $conn -> query($query);
