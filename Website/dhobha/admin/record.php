...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(!empty($_POST["date"]) && !empty($_POST["desc"])){
        ...
        if(!$conn){
            ...
            $query = "select * from dexpense order by tamount desc limit 1";
            ...
            $query = "insert into dexpense(ddate, ddesc, descOp1, descOp2, descOp3, descOp4, descOp5, amount, damount, tamount) values('$date', '$desc', '$descOp1', '$descOp2', '$descOp3', '$descOp4', '$descOp5', '$amount', '$damount', '$total_amount')";
            if(!$conn -> query($query)){
